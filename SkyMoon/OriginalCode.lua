-- 🌙 SkyMoon ScriptHub | Original Code
...
for i = 0, 20 do
    ...
    l.Position = UDim2.new(0,0,0,i*40)
    l.BackgroundColor3 = Color3.fromRGB(18,12,40)
...
for i = 0, 30 do
    ...
    l.Position = UDim2.new(0,i*50,0,0)
    l.BackgroundColor3 = Color3.fromRGB(18,12,40)
    l.BackgroundTransparency = 0.7
    l.BorderSizePixel = 0
end
...
center.Size = UDim2.new(0,280,0,210)
center.Position = UDim2.new(0.5,-140,0.5,-105)
center.BackgroundTransparency = 1
center.BorderSizePixel = 0

local ro = Instance.new("ImageLabel", center)
ro.Size = UDim2.new(0,90,0,90)
ro.Position = UDim2.new(0.5,-45,0,0)
ro.BackgroundTransparency = 1
ro.Image = "rbxassetid://6031094678"
ro.ImageColor3 = Color3.fromRGB(110,70,255)
ro.ImageTransparency = 0.2

local ri = Instance.new("ImageLabel", center)
ri.Size = UDim2.new(0,60,0,60)
ri.Position = UDim2.new(0.5,-30,0,15)
ri.BackgroundTransparency = 1
ri.Image = "rbxassetid://6031094678"
ri.ImageColor3 = Color3.fromRGB(0,200,170)
ri.ImageTransparency = 0.4

local moonLbl = Instance.new("TextLabel", center)
moonLbl.Size = UDim2.new(0,90,0,90)
moonLbl.Position = UDim2.new(0.5,-45,0,0)
...
titleL.Position = UDim2.new(0,0,0,96)
...
subL.Position = UDim2.new(0,0,0,126)
subL.BackgroundTransparency = 1
subL.Text = "SCRIPT HUB  ·  BY KHAFIDZKTP"
subL.Font = Enum.Font.GothamBold
subL.TextSize = 9
subL.TextColor3 = Color3.fromRGB(60,50,100)

local barBg = Instance.new("Frame", center)
barBg.Size = UDim2.new(1,0,0,4)
barBg.Position = UDim2.new(0,0,0,152)
barBg.BackgroundColor3 = Color3.fromRGB(20,15,40)
barBg.BorderSizePixel = 0
Instance.new("UICorner", barBg).CornerRadius = UDim.new(0,2)

local barFill = Instance.new("Frame", barBg)
barFill.Size = UDim2.new(0,0,1,0)
barFill.BackgroundColor3 = Color3.fromRGB(110,70,255)
...
statusL.Position = UDim2.new(0,0,0,160)
...
local function openScanOverlay()
    ...
    img.Position = UDim2.new(0.5,-20,0,7)
    ...
    local function typeText(text, color, delay)
        for i = 1, #text do
            output.Text = output.Text .. string.format(
                '<font color="#%s">%s</font>', color or "00ff44", escapeRich(text:sub(i,i)))
    ...
    task.spawn(function()
        ...
        for i = 1, 6 do
            appendLine(string.format("[%s] %d.%d.%d.%d  CONNECTED",
                ...
                math.random(0,255), math.random(1,254)), "00aa55")
            task.wait(0.08)
        end
        ...
        for i, item in ipairs(allItems) do
            ...
            if #lineBuffer >= 4 or item.label ~= lastLabel then
                appendLine(string.format("[KHAFIDZKTP, %s] %s  (%d%%)",
                    item.label, table.concat(lineBuffer, " | "), pct), "00ff44")
                lineBuffer = {}
            ...
            for pid, data in pairs(db) do
                ...
                if ok and ok2 and info and myInfo and info.Creator and myInfo.Creator then
                    if info.Creator.Id == myInfo.Creator.Id then
                        entry = data
                        appendLine("✓ Sub-place — same game verified", "ffaa00")
                        break
                    end
                end
            end
...
local function openMiniCmd()
    ...
    ws.Color = Color3.fromRGB(60,50,160)
    ws.Thickness = 1.5

    local tbar = Instance.new("Frame", win)
    tbar.Size = UDim2.new(1,0,0,28)
    tbar.BackgroundColor3 = Color3.fromRGB(6,6,14)
    ...
    tf.BackgroundColor3 = Color3.fromRGB(6,6,14)
    ...
    tl.Text = "🌙 SkyMoon Terminal"
    tl.Font = Enum.Font.GothamBold
    tl.TextSize = 11
    tl.TextColor3 = Color3.fromRGB(150,130,255)
    tl.TextXAlignment = Enum.TextXAlignment.Left

    local cb = Instance.new("TextButton", tbar)
    cb.Size = UDim2.new(0,22,0,20)
    cb.Position = UDim2.new(1,-26,0,4)
    cb.BackgroundColor3 = Color3.fromRGB(160,30,50)
    cb.Text = "✕"
    ...
    local outL = Instance.new("TextLabel", scroll)
    outL.Size = UDim2.new(1,-4,0,0)
    outL.AutomaticSize = Enum.AutomaticSize.Y
    outL.BackgroundTransparency = 1
    outL.Text = '<font color="#555577">SkyMoon CMD ready.\n</font>'
    outL.Font = Enum.Font.Code
    outL.TextSize = 11
    outL.TextColor3 = Color3.fromRGB(200,210,255)
    outL.TextXAlignment = Enum.TextXAlignment.Left
    outL.TextYAlignment = Enum.TextYAlignment.Top
    outL.TextWrapped = true
    outL.RichText = true
    ...
    local ibar = Instance.new("Frame", win)
    ibar.Size = UDim2.new(1,-12,0,26)
    ibar.Position = UDim2.new(0,6,1,-30)
    ibar.BackgroundColor3 = Color3.fromRGB(16,14,32)
    ibar.BorderSizePixel = 0
    Instance.new("UICorner", ibar).CornerRadius = UDim.new(0,6)
    local ibs = Instance.new("UIStroke", ibar)
    ibs.Color = Color3.fromRGB(60,50,160)

    local prompt = Instance.new("TextLabel", ibar)
    ...
    local inputBox = Instance.new("TextBox", ibar)
    ...
    inputBox.FocusLost:Connect(function(enter)
        ...
        elseif lower:match("^check in") then
            ...
            local parts = {}
            for p in inner:gmatch("[^,]+") do table.insert(parts, p:match("^%s*(.-)%s*$")) end
            ...
            for i = 2, #parts do
                ...
                if not cur then appendOut("Not found: " .. parts[i], "ff6644") inputBox.Text="" return end
            end
...
local function openConsole()
    ...
    local win = Instance.new("Frame", cSg)
    win.Size = UDim2.new(0,500,0,300)
    win.Position = UDim2.new(0.5,-250,0.5,-150)
    win.BackgroundColor3 = Color3.fromRGB(8,8,16)
    win.BorderSizePixel = 0
    win.Active = true
    win.Draggable = true
    ...
    ws2.Color = Color3.fromRGB(60,50,160)
    ws2.Thickness = 1.5

    local tbar2 = Instance.new("Frame", win)
    tbar2.Size = UDim2.new(1,0,0,28)
    tbar2.BackgroundColor3 = Color3.fromRGB(6,6,14)
    tbar2.BorderSizePixel = 0
    Instance.new("UICorner", tbar2).CornerRadius = UDim.new(0,10)
    local tf2 = Instance.new("Frame", tbar2)
    tf2.Size = UDim2.new(1,0,0.5,0)
    tf2.Position = UDim2.new(0,0,0.5,0)
    tf2.BackgroundColor3 = Color3.fromRGB(6,6,14)
    tf2.BorderSizePixel = 0

    local tl2 = Instance.new("TextLabel", tbar2)
    tl2.Size = UDim2.new(1,-50,1,0)
    tl2.Position = UDim2.new(0,10,0,0)
    tl2.BackgroundTransparency = 1
    tl2.Text = "🌙 Live Console"
    tl2.Font = Enum.Font.GothamBold
    tl2.TextSize = 11
    tl2.TextColor3 = Color3.fromRGB(150,130,255)
    tl2.TextXAlignment = Enum.TextXAlignment.Left

    local cb2 = Instance.new("TextButton", tbar2)
    cb2.Size = UDim2.new(0,22,0,20)
    cb2.Position = UDim2.new(1,-26,0,4)
    cb2.BackgroundColor3 = Color3.fromRGB(160,30,50)
    cb2.Text = "✕"
    cb2.TextColor3 = Color3.new(1,1,1)
    cb2.Font = Enum.Font.GothamBold
    cb2.TextSize = 10
    cb2.BorderSizePixel = 0
    Instance.new("UICorner", cb2).CornerRadius = UDim.new(0,4)
    cb2.MouseButton1Click:Connect(function() cSg:Destroy() end)

    local scroll2 = Instance.new("ScrollingFrame", win)
    scroll2.Size = UDim2.new(1,-12,1,-36)
    scroll2.Position = UDim2.new(0,6,0,32)
    scroll2.BackgroundTransparency = 1
    scroll2.BorderSizePixel = 0
    scroll2.ScrollBarThickness = 2
    scroll2.AutomaticCanvasSize = Enum.AutomaticSize.Y
    scroll2.CanvasSize = UDim2.new(0,0,0,0)

    local outL2 = Instance.new("TextLabel", scroll2)
    outL2.Size = UDim2.new(1,-4,0,0)
    outL2.AutomaticSize = Enum.AutomaticSize.Y
    outL2.BackgroundTransparency = 1
    outL2.Text = ""
    outL2.Font = Enum.Font.Code
    outL2.TextSize = 11
    outL2.TextColor3 = Color3.fromRGB(200,210,255)
    outL2.TextXAlignment = Enum.TextXAlignment.Left
    outL2.TextYAlignment = Enum.TextYAlignment.Top
    outL2.TextWrapped = true
    outL2.RichText = true
    ...
    local logConn = game:GetService("LogService").MessageOut:Connect(function(msg, msgType)
        ...
        outL2.Text = outL2.Text .. string.format('<font color="#%s">%s\n</font>', color, escapeRich(msg:sub(1,120)))
        task.defer(function() scroll2.CanvasPosition = Vector2.new(0, scroll2.AbsoluteCanvasSize.Y) end)
    end)
...
local function startWindUI()
    ...
    if not ok then
        warn("[SkyMoon] WindUI failed: " .. tostring(lib))
        return
    end
    WindUI = lib
    ...
    HomeTab:Paragraph({
        ...
        Content = "Welcome! Use Scan Game to detect and run scripts for the current game.\n\nChat commands: /Open_Cmd  /console  /reset_skymoon"
    })
    HomeTab:Divider()
    HomeTab:Button({ Title="⟳ Scan Game", Desc="Detect game and run matching script",
        Callback = function()
            Window:Close()
            task.wait(0.2)
            openScanOverlay()
    ...
    HomeTab:Button({ Title="📋 Universal Scripts", Desc="Browse scripts for any game",
        Callback = function()
            local udb = fetchUniversal()
            if not udb then notify("❌ Error","Failed to load universal list!") return end
            local names = {}
            for k, v in pairs(udb) do table.insert(names, v.name) end
            notify("Universal Scripts", table.concat(names, ", "):sub(1,100), 5)
        end
    })
    HomeTab:Button({ Title="🔥 Real Builder", Desc="Open in-game Studio builder",
        Callback = function()
            notify("⏳ Loading", "Loading Real Builder...")
            task.spawn(function() runScript(RealBuilder_URL) end)
        end
    })
    HomeTab:Button({ Title="💻 Open Terminal", Desc="Mini CMD — deep workspace scan",
        Callback = function() openMiniCmd() end
    })
    HomeTab:Button({ Title="📟 Open Console", Desc="Live output console",
    ...
    PT:Toggle({ Title="God Mode", Value=false, Callback=function(s)
        pcall(function()
            local h = LP.Character.Humanoid
            if s then h.MaxHealth = math.huge h.Health = math.huge
            else h.MaxHealth = 100 h.Health = 100 end
        end)
    end})
    PT:Toggle({ Title="NoClip", Value=false, Callback=function(s)
        if s then
            _G.SkyNC = RunService.Stepped:Connect(function()
                pcall(function()
            ...
            if _G.SkyNC then _G.SkyNC:Disconnect() _G.SkyNC = nil end
        end
    end})
    PT:Toggle({ Title="Infinite Jump", Value=false, Callback=function(s)
        if s then
            _G.SkyIJ = UIS.JumpRequest:Connect(function()
                pcall(function() LP.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping) end)
            end)
        else
            if _G.SkyIJ then _G.SkyIJ:Disconnect() _G.SkyIJ = nil end
        end
    end})
    ...
    PT:Toggle({ Title="Invisible (local)", Value=false, Callback=function(s)
        pcall(function()
            for _, p in ipairs(LP.Character:GetDescendants()) do
    ...
    MT:Slider({ Title="Gravity", Step=1, Value={Min=0, Max=600, Default=196},
    ...
    MT:Button({ Title="🚀 Launch Up", Callback=function()
        pcall(function()
    ...
    MT:Button({ Title="💨 Dash Forward", Callback=function()
        pcall(function()
            local hrp = LP.Character.HumanoidRootPart
    ...
    BT:Button({ Title="⟳ Spawn Part", Callback=function()
        pcall(function()
            local p = Instance.new("Part")
            p.CFrame = workspace.CurrentCamera.CFrame * CFrame.new(0,0,-8)
    ...
    BT:Button({ Title="🔥 Real Builder", Desc="Full in-game Studio mode", Callback=function()
        notify("⏳ Loading", "Loading Real Builder...")
        task.spawn(function() runScript(RealBuilder_URL) end)
    end})
    BT:Toggle({ Title="Freeze All Parts", Value=false, Callback=function(s)
        pcall(function()
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj:IsA("BasePart") then obj.Anchored = s end
            end
        end)
        notify(s and "🔒 Frozen" or "🔓 Unfrozen", "All parts " .. (s and "anchored" or "unanchored") .. "!")
    end})
    ...
    TPT:Input({ Title="Teleport XYZ", Placeholder="X,Y,Z  e.g. 0,50,0", Callback=function(text)
        local c = {}
        for n in text:gmatch("[%-]?%d+%.?%d*") do table.insert(c, tonumber(n)) end
        if #c >= 3 then
            pcall(function() LP.Character.HumanoidRootPart.CFrame = CFrame.new(c[1],c[2],c[3]) end)
            notify("📍 TP", "Teleported!")
        else
            notify("⚠️", "Enter valid X,Y,Z")
        end
    end})
    TPT:Button({ Title="Save Position", Callback=function()
        _G.SkyPos = LP.Character.HumanoidRootPart.CFrame
        notify("📍 Saved", "Position saved!")
    end})
    TPT:Button({ Title="Load Position", Callback=function()
        if _G.SkyPos then
            pcall(function() LP.Character.HumanoidRootPart.CFrame = _G.SkyPos end)
        else notify("⚠️", "No saved position!") end
    end})
    TPT:Button({ Title="🌍 Map Center (0,50,0)", Callback=function()
        pcall(function() LP.Character.HumanoidRootPart.CFrame = CFrame.new(0,50,0) end)
    end})
    TPT:Button({ Title="🗺️ Random Location", Callback=function()
        pcall(function()
    ...
    TPT:Dropdown({ Title="Teleport to Player",
        Values = function()
            local n = {}
        ...
        Callback = function(name)
            ...
            if t and t.Character then
                pcall(function()
                    LP.Character.HumanoidRootPart.CFrame =
                        t.Character.HumanoidRootPart.CFrame + Vector3.new(3,0,0)
                end)
    ...
    GT:Toggle({ Title="Hide All GUIs", Value=false, Callback=function(s)
        pcall(function()
            for _, g in ipairs(LP.PlayerGui:GetChildren()) do
                if g:IsA("ScreenGui") and not g.Name:find("SkyMoon") then
                    g.Enabled = not s
                end
            end
        end)
    end})
    GT:Button({ Title="📋 List GUIs", Callback=function()
        local n = {}
        pcall(function()
            for _, g in ipairs(LP.PlayerGui:GetChildren()) do table.insert(n, g.Name) end
        end)
        notify("GUIs (" .. #n .. ")", table.concat(n, ", "):sub(1,120), 5)
    end})
    GT:Button({ Title="🗑️ Delete Non-SkyMoon GUIs", Callback=function()
        pcall(function()
            for _, g in ipairs(LP.PlayerGui:GetChildren()) do
                if g:IsA("ScreenGui") and not g.Name:find("SkyMoon") then g:Destroy() end
            end
        end)
        notify("🗑️ Done", "Non-SkyMoon GUIs removed!")
    end})
    ...
    SndT:Toggle({ Title="Mute All Sounds", Value=false, Callback=function(s)
        pcall(function()
            for _, so in ipairs(workspace:GetDescendants()) do
                if so:IsA("Sound") then so.Volume = s and 0 or 1 end
            end
        end)
        notify(s and "🔇 Muted" or "🔊 Unmuted", "All sounds " .. (s and "muted" or "restored") .. "!")
    end})
    SndT:Slider({ Title="Master Volume", Step=0.05, Value={Min=0, Max=1, Default=1},
        Callback=function(v)
            pcall(function()
                for _, so in ipairs(workspace:GetDescendants()) do
                    if so:IsA("Sound") then so.Volume = v end
                end
            end)
        end
    })
    ...
    SndT:Input({ Title="Set Song ID", Placeholder="Paste Sound ID, press Enter",
        Callback=function(text)
            local id = text:match("%d+")
            if id then _G.SkySongId = id notify("🎵 ID Set", "Sound ID: " .. id) end
        end
    })
    SndT:Dropdown({ Title="Play Location", Values={"Workspace","Camera","LocalPlayer"},
        Callback=function(c) _G.SkySndTarget = c end
    })
    SndT:Slider({ Title="Song Volume", Step=0.05, Value={Min=0, Max=1, Default=0.8},
        Callback=function(v)
            _G.SkySndVol = v
            if csnd then pcall(function() csnd.Volume = v end) end
        end
    })
    SndT:Button({ Title="▶ Play Song", Callback=function()
        local id = tostring(_G.SkySongId or "")
        if id == "" then notify("⚠️", "Enter a Sound ID first!") return end
        if csnd then pcall(function() csnd:Stop() csnd:Destroy() end) csnd = nil end
        pcall(function()
            local s = Instance.new("Sound")
            s.SoundId = "rbxassetid://" .. id
            s.Volume = _G.SkySndVol or 0.8
            s.Looped = true
            local target = workspace
            if _G.SkySndTarget == "Camera" then target = workspace.CurrentCamera
            elseif _G.SkySndTarget == "LocalPlayer" then target = LP end
            s.Parent = target
            s:Play()
            csnd = s
            notify("🎵 Playing", "ID: " .. id)
        end)
    end})
    SndT:Button({ Title="⏸ Pause", Callback=function()
        if csnd then pcall(function() csnd:Pause() end)
        else notify("⚠️", "No song playing!") end
    end})
    SndT:Button({ Title="▶ Resume", Callback=function()
        if csnd then pcall(function() csnd:Resume() end)
        else notify("⚠️", "No song playing!") end
    end})
    SndT:Button({ Title="⏹ Stop", Callback=function()
        if csnd then
            pcall(function() csnd:Stop() csnd:Destroy() end)
            csnd = nil
            notify("⏹ Stopped", "Song stopped.")
        else notify("⚠️", "No song playing!") end
    end})
    SndT:Toggle({ Title="Loop Song", Value=true, Callback=function(s)
        if csnd then pcall(function() csnd.Looped = s end) end
    end})
    SndT:Button({ Title="🎵 Ambient BG", Desc="Play SkyMoon background sound", Callback=function()
        pcall(function()
            local s = Instance.new("Sound")
    ...
    SetT:Paragraph({
        Title   = "About SkyMoon",
        Content = "Version: 3.0 (WindUI + Junkie)\nAuthor: KHAFIDZKTP\nGitHub: github.com/HaZcK/ScriptHub"
    })
    SetT:Divider()
