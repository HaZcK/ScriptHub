...
local function startFreeCam()
    ...
    -- NoClip (tetap supaya char gak nabrak tanah)
...
----------------------------------------------------
-- RAYCAST SELECTION (click pada part di viewport)
----------------------------------------------------
local function trySelectFromRaycast(screenPos)
    -- Jangan select kalau RMB held (sedang look)
    ...
    -- Exclude handles, char, dan CoreGui descendants
    local exclude = {}
    ...
    if result and result.Instance then
        ...
        -- Naik ke parent kalau obj adalah handle
...
----------------------------------------------------
-- DRAG TO MOVE/SCALE/ROTATE (mouse + touch)
----------------------------------------------------
local isDragging = false
local lastInputPos = nil
local dragStarted = false
local clickStartPos = nil
local CLICK_THRESHOLD = 6 -- pixels, kalau drag < ini = click biasa
...
UIS.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1
    or inp.UserInputType == Enum.UserInputType.Touch then
        -- Kalau belum drag = klik → raycast select
...
UIS.InputChanged:Connect(function(inp)
    ...
    -- Cek apakah ini drag beneran
    if not dragStarted then
        local totalDelta = (inp.Position - clickStartPos).Magnitude
        if totalDelta < CLICK_THRESHOLD then return end
        dragStarted = true
    end
    ...
    pcall(function()
        if currentTool == "Move" then
            ...
            selectedObj.CFrame = selectedObj.CFrame
                + cam.CFrame.RightVector * delta.X * 0.12
                + cam.CFrame.UpVector * (-delta.Y) * 0.12
            updateHandles()
        elseif currentTool == "Scale" then
