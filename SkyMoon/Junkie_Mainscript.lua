-- 🌙 SkyMoon ScriptHub | Junkie Edition
-- by KHAFIDZKTP | Paste this ENTIRE file into Junkie Lua Script editor
...
local ro=Instance.new("ImageLabel",center) ro.Size=UDim2.new(0,90,0,90) ro.Position=UDim2.new(0.5,-45,0,0)
ro.BackgroundTransparency=1 ro.Image="rbxassetid://6031094678" ro.ImageColor3=Color3.fromRGB(110,70,255) ro.ImageTransparency=0.2
local ri=Instance.new("ImageLabel",center) ri.Size=UDim2.new(0,60,0,60) ri.Position=UDim2.new(0.5,-30,0,15)
ri.BackgroundTransparency=1 ri.Image="rbxassetid://6031094678" ri.ImageColor3=Color3.fromRGB(0,200,170) ri.ImageTransparency=0.4
local moonLbl=Instance.new("TextLabel",center) moonLbl.Size=UDim2.new(0,90,0,90) moonLbl.Position=UDim2.new(0.5,-45,0,0)
moonLbl.BackgroundTransparency=1 moonLbl.Text="🌙" moonLbl.Font=Enum.Font.GothamBold moonLbl.TextSize=34

local titleL=Instance.new("TextLabel",center) titleL.Size=UDim2.new(1,0,0,30) titleL.Position=UDim2.new(0,0,0,96)
titleL.BackgroundTransparency=1 titleL.Text="SKYMOON" titleL.Font=Enum.Font.GothamBold titleL.TextSize=22
titleL.TextColor3=Color3.fromRGB(160,130,255)
local subL=Instance.new("TextLabel",center) subL.Size=UDim2.new(1,0,0,16) subL.Position=UDim2.new(0,0,0,126)
subL.BackgroundTransparency=1 subL.Text="SCRIPT HUB  ·  BY KHAFIDZKTP" subL.Font=Enum.Font.GothamBold subL.TextSize=9
subL.TextColor3=Color3.fromRGB(60,50,100)

local barBg=Instance.new("Frame",center) barBg.Size=UDim2.new(1,0,0,4) barBg.Position=UDim2.new(0,0,0,152)
...
barFill.BackgroundColor3=Color3.fromRGB(110,70,255) barFill.BorderSizePixel=0
...
local statusL=Instance.new("TextLabel",center) statusL.Size=UDim2.new(1,0,0,14) statusL.Position=UDim2.new(0,0,0,160)
statusL.BackgroundTransparency=1 statusL.Text="Initializing..." statusL.Font=Enum.Font.Code statusL.TextSize=10
statusL.TextColor3=Color3.fromRGB(70,60,110)

local spinConn=RunService.RenderStepped:Connect(function(dt)
    ro.Rotation=ro.Rotation+80*dt ri.Rotation=ri.Rotation-120*dt
end)

local STEPS={
    {0.15,"Loading WindUI library..."},
    {0.30,"Connecting to ScriptHub..."},
    {0.50,"Verifying key..."},
    {0.68,"Fetching game database..."},
    {0.85,"Preparing admin panel..."},
    {1.00,"Ready!"},
}

local function runLoadingSequence(onComplete)
    local stepTime=2.5/#STEPS
...
local function fetchPlaceList()
    local raw=httpGet(RAW_PLACELIST) if not raw then return nil end
    local ok,db=pcall(function() return HttpService:JSONDecode(raw) end)
    return ok and db or nil
end
local function fetchUniversal()
    local raw=httpGet(RAW_UNIVERSAL) if not raw then return nil end
    local ok,db=pcall(function() return HttpService:JSONDecode(raw) end)
    return ok and db or nil
end
...
local function notify(title,msg,dur)
    pcall(function() WindUI:Notify({Title=title,Content=msg,Duration=dur or 4}) end)
end
...
local function openScanOverlay()
    local scanSg=Instance.new("ScreenGui") scanSg.Name="SkyMoon_Scan" scanSg.ResetOnSpawn=false
    pcall(function() scanSg.Parent=game:GetService("CoreGui") end)
    if not scanSg.Parent then scanSg.Parent=LP.PlayerGui end
    local bg=Instance.new("Frame",scanSg) bg.Size=UDim2.new(1,0,1,0)
    bg.BackgroundColor3=Color3.fromRGB(0,0,0) bg.BackgroundTransparency=0.05 bg.BorderSizePixel=0
    local header=Instance.new("Frame",bg) header.Size=UDim2.new(1,0,0,54)
    header.BackgroundColor3=Color3.fromRGB(10,10,18) header.BorderSizePixel=0
    local img=Instance.new("ImageLabel",header) img.Size=UDim2.new(0,40,0,40)
    img.Position=UDim2.new(0.5,-20,0,7) img.BackgroundTransparency=1
    img.Image="https://tkj.smkdarmasiswasidoarjo.sch.id/wp-content/uploads/2024/08/61ef634e-0b5f-4d27-9fb6-c64d526c595c.png"
    local output=Instance.new("TextLabel",bg) output.Size=UDim2.new(1,-32,1,-60)
    output.Position=UDim2.new(0,16,0,58) output.BackgroundTransparency=1 output.Font=Enum.Font.Code
    output.TextSize=13 output.TextColor3=Color3.fromRGB(0,255,80) output.TextXAlignment=Enum.TextXAlignment.Left
    output.TextYAlignment=Enum.TextYAlignment.Top output.TextWrapped=true output.RichText=true output.Text=""
    ...
    local function typeText(text,color,delay)
        for i=1,#text do output.Text=output.Text..string.format('<font color="#%s">%s</font>',color or "00ff44",escapeRich(text:sub(i,i))) task.wait(delay or 0.04) end
        output.Text=output.Text.."\n"
    end
    task.spawn(function()
        appendLine("SkyMoon Terminal v3.0 — WindUI Edition","aaaaff")
        ...
        for i=1,6 do
            appendLine(string.format("[%s] %d.%d.%d.%d  CONNECTED",
                countries[math.random(1,#countries)],math.random(1,255),math.random(0,255),math.random(0,255),math.random(1,254)),"00aa55")
            task.wait(0.08)
        end
        output.Text=""
        typeText("Check_This_Game:;","00ff88",0.06)
        task.wait(0.2)
        local SERVICES={{game:GetService("Workspace"),"Workspace"},{game:GetService("ReplicatedStorage"),"ReplicatedStorage"},{LP,"LocalPlayer"}}
        local allItems={}
        ...
        for _,s in ipairs(SERVICES) do pcall(function() collect(s[1],0,s[2]) end) end
        ...
        for i,item in ipairs(allItems) do
            ...
            if #lineBuffer>=4 or item.label~=lastLabel then
                appendLine(string.format("[KHAFIDZKTP, %s] %s  (%d%%)",item.label,table.concat(lineBuffer," | "),pct),"00ff44")
                lineBuffer={} lineCount=lineCount+1
            end
        ...
        output.Text="" appendLine("Check_This_Game... 100%","00ffaa") task.wait(0.4)
        ...
        if not entry and db then
            for pid,data in pairs(db) do
                ...
                if ok and ok2 and info and myInfo and info.Creator and myInfo.Creator then
                    if info.Creator.Id==myInfo.Creator.Id then entry=data appendLine("✓ Sub-place — same game verified","ffaa00") break end
                end
            end
        end
        task.wait(0.3)
        if entry then
            appendLine("✓ Game supported! Running script...","00ffaa") task.wait(0.5)
            scanSg:Destroy() runScript(entry.script)
        else
            appendLine("✗ Game not supported.","ff4444") task.wait(0.3)
            typeText("Loading Universal Scripts...","ffaa00",0.05) task.wait(0.3)
            local udb=fetchUniversal()
            if udb then
                output.Text="" appendLine("=== Universal Script List ===","aaaaff")
                for k,v in pairs(udb) do appendLine(string.format("[%s] %s",k,v.name),"88ffbb") end
                task.wait(3)
            end
            scanSg:Destroy()
        end
...
local function openMiniCmd()
    ...
    pcall(function() cSg.Parent=game:GetService("CoreGui") end) if not cSg.Parent then cSg.Parent=LP.PlayerGui end
    ...
    Instance.new("UIStroke",win).Color=Color3.fromRGB(60,50,160)
    ...
    scroll.BackgroundTransparency=1 scroll.BorderSizePixel=0 scroll.ScrollBarThickness=2 scroll.AutomaticCanvasSize=Enum.AutomaticSize.Y scroll.CanvasSize=UDim2.new(0,0,0,0)
    ...
    inputBox.BackgroundTransparency=1 inputBox.Text="" inputBox.PlaceholderText="Check In [Workspace, Folder, Name]  |  RunConsole"
    ...
    inputBox.FocusLost:Connect(function(enter)
        ...
        elseif lower:match("^check in") then
            ...
            local cur=svcs[parts[1]]
            if not cur then appendOut("Unknown service: "..parts[1],"ff6644") inputBox.Text="" return end
...
local function openConsole()
    ...
    local cb2=Instance.new("TextButton",tbar) cb2.Size=UDim2.new(0,22,0,20) cb2.Position=UDim2.new(1,-26,0,4) cb2.BackgroundColor3=Color3.fromRGB(160,30,50)
    cb2.Text="✕" cb2.TextColor3=Color3.new(1,1,1) cb2.Font=Enum.Font.GothamBold cb2.TextSize=10 cb2.BorderSizePixel=0
    Instance.new("UICorner",cb2).CornerRadius=UDim.new(0,4) cb2.MouseButton1Click:Connect(function() cSg:Destroy() end)
...
----------------------------------------------------
-- WINDUI HUB (called after loading + key verified)
----------------------------------------------------
local function startWindUI()
    local ok,lib=pcall(function()
        return loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"))()
    end)
    if not ok then warn("[SkyMoon] WindUI load failed: "..tostring(lib)) return end
    WindUI=lib
    ...
    HomeTab:Paragraph({Title="🌙 SkyMoon Script Hub",Content="Welcome! Use ⟳ Scan Game to detect and run scripts.\n\nChat: /Open_Cmd  /console  /reset_skymoon"})
    HomeTab:Divider()
    HomeTab:Button({Title="⟳ Scan Game",Desc="Detect current game and execute matching script",Callback=function()
        Window:Close() task.wait(0.2) openScanOverlay()
    end})
    HomeTab:Button({Title="📋 Universal Scripts",Desc="Browse scripts for any game",Callback=function()
        local udb=fetchUniversal() if not udb then notify("❌ Error","Failed to load!") return end
        local names={} for k,v in pairs(udb) do table.insert(names,v.name) end
        notify("Universal Scripts","Found: "..table.concat(names,", "):sub(1,100),5)
    end})
    HomeTab:Button({Title="🔥 Real Builder",Desc="Open in-game Studio builder",Callback=function()
        notify("⏳ Loading","Loading Real Builder...")
        task.spawn(function() runScript(RealBuilder_URL) end)
    end})
    HomeTab:Button({Title="💻 Open Terminal",Desc="Mini CMD",Callback=function() openMiniCmd() end})
    HomeTab:Button({Title="📟 Open Console",Desc="Live output",Callback=function() openConsole() end})
    HomeTab:Divider()
    HomeTab:Paragraph({Title="Game Info",Content="PlaceId: "..game.PlaceId.."\nPlayer: "..LP.Name})
    ...
    PT:Button({Title="Save Position",Callback=function() _G.SkyPos=LP.Character.HumanoidRootPart.CFrame notify("📍 Saved","Position saved!") end})
    ...
    PT:Input({Title="Set Nametag",Placeholder="Display name...",Callback=function(text)
    ...
    BT:Button({Title="⟳ Spawn Part",Callback=function() pcall(function() local p=Instance.new("Part") p.CFrame=workspace.CurrentCamera.CFrame*CFrame.new(0,0,-8) p.Size=Vector3.new(4,4,4) p.Anchored=true p.BrickColor=BrickColor.new("Bright blue") p.Parent=workspace end) notify("📦 Spawned","Part spawned!") end})
    BT:Button({Title="🔥 Real Builder",Desc="Full in-game Studio mode",Callback=function() notify("⏳ Loading","Loading Real Builder...") task.spawn(function() runScript(RealBuilder_URL) end) end})
    BT:Toggle({Title="Freeze All Parts",Value=false,Callback=function(s) pcall(function() for _,obj in ipairs(workspace:GetDescendants()) do if obj:IsA("BasePart") then obj.Anchored=s end end end) notify(s and "🔒 Frozen" or "🔓 Unfrozen","All parts "..(s and "anchored" or "unanchored").."!") end})

    -- TELEPORT
    local TPT=Window:Tab({Title="TP",Icon="map-pin"})
    TPT:Input({Title="Teleport XYZ",Placeholder="X,Y,Z  e.g. 0,50,0",Callback=function(text)
        local c={} for n in text:gmatch("[%-]?%d+%.?%d*") do table.insert(c,tonumber(n)) end
        if #c>=3 then pcall(function() LP.Character.HumanoidRootPart.CFrame=CFrame.new(c[1],c[2],c[3]) end) notify("📍 TP","Teleported!")
        else notify("⚠️","Enter valid X,Y,Z") end
    end})
    TPT:Button({Title="Save Position",Callback=function() _G.SkyPos=LP.Character.HumanoidRootPart.CFrame notify("📍 Saved","Position saved!") end})
    ...
    TPT:Button({Title="🗺️ Random Location",Callback=function() pcall(function() local x,z=math.random(-500,500),math.random(-500,500) LP.Character.HumanoidRootPart.CFrame=CFrame.new(x,50,z) notify("🗺️ TP","X:"..x.." Z:"..z) end) end})
    TPT:Dropdown({Title="Teleport to Player",Values=function() local n={} for _,p in ipairs(Players:GetPlayers()) do if p~=LP then table.insert(n,p.Name) end end return n end,
        Callback=function(name) local t=Players:FindFirstChild(name) if t and t.Character then pcall(function() LP.Character.HumanoidRootPart.CFrame=t.Character.HumanoidRootPart.CFrame+Vector3.new(3,0,0) end) notify("📍 TP","Teleported to "..name) end end})
    ...
    GT:Button({Title="🗑️ Delete All GUIs",Callback=function() pcall(function() for _,g in ipairs(LP.PlayerGui:GetChildren()) do if g:IsA("ScreenGui") and not g.Name:find("SkyMoon") then g:Destroy() end end end) notify("🗑️ Done","Non-SkyMoon GUIs removed!") end})

    -- SOUND
    local ST=Window:Tab({Title="Sound",Icon="music"})
    local csnd=nil
    ST:Toggle({Title="Mute All Sounds",Value=false,Callback=function(s) pcall(function() for _,so in ipairs(workspace:GetDescendants()) do if so:IsA("Sound") then so.Volume=s and 0 or 1 end end end) notify(s and "🔇 Muted" or "🔊 Unmuted","All sounds "..(s and "muted" or "restored").."!") end})
    ST:Slider({Title="Master Volume",Step=0.05,Value={Min=0,Max=1,Default=1},Callback=function(v) pcall(function() for _,so in ipairs(workspace:GetDescendants()) do if so:IsA("Sound") then so.Volume=v end end end) end})
    ST:Divider() ST:Section({Title="Custom Song"})
    ST:Input({Title="Set Song ID",Placeholder="Paste Sound ID, press Enter",Callback=function(text) local id=text:match("%d+") if id then _G.SkySongId=id notify("🎵 ID Set","Sound ID: "..id) end end})
    ST:Dropdown({Title="Play Location",Values={"Workspace","Camera","LocalPlayer"},Callback=function(c) _G.SkySndTarget=c end})
    ST:Slider({Title="Song Volume",Step=0.05,Value={Min=0,Max=1,Default=0.8},Callback=function(v) _G.SkySndVol=v if csnd then pcall(function() csnd.Volume=v end) end end})
    ST:Button({Title="▶ Play Song",Callback=function()
        local id=tostring(_G.SkySongId or "") if id=="" then notify("⚠️","Enter a Sound ID first!") return end
    ...
    ST:Button({Title="⏸ Pause",Callback=function() if csnd then pcall(function() csnd:Pause() end) else notify("⚠️","No song playing!") end end})
    ST:Button({Title="▶ Resume",Callback=function() if csnd then pcall(function() csnd:Resume() end) else notify("⚠️","No song playing!") end end})
    ST:Button({Title="⏹ Stop",Callback=function() if csnd then pcall(function() csnd:Stop() csnd:Destroy() end) csnd=nil notify("⏹ Stopped","Song stopped.") else notify("⚠️","No song playing!") end end})
    ST:Toggle({Title="Loop Song",Value=true,Callback=function(s) if csnd then pcall(function() csnd.Looped=s end) end end})
    ST:Button({Title="🎵 Ambient BG",Desc="Play SkyMoon background sound",Callback=function()
        pcall(function() local s=Instance.new("Sound") s.SoundId="rbxassetid://139132289200391" s.Volume=0.4 s.Looped=true s.Parent=workspace s:Play() csnd=s end)
        notify("🎵 Ambient","Background sound playing!")
    end})

    -- SETTINGS
    local SetT=Window:Tab({Title="Settings",Icon="settings"})
    SetT:Paragraph({Title="About SkyMoon",Content="Version: 3.0 (WindUI + Junkie)\nAuthor: KHAFIDZKTP\nGitHub: github.com/HaZcK/ScriptHub"})
    SetT:Divider()
    ...
    SetT:Button({Title="📋 Copy PlaceId",Callback=function() pcall(function() setclipboard(tostring(game.PlaceId)) end) notify("📋 Copied","PlaceId: "..game.PlaceId) end})

    -- Load tools in background
    task.spawn(function()
        local ok2,res=pcall(function() return game:HttpGet(ALL_TOOLS_URL) end)
        if ok2 and res and res~="" then local fn=loadstring(res) if fn then pcall(fn) end end
    end)
