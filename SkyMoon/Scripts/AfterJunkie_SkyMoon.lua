-- ════════════════════════════════════════════════
-- 🌙 SkyMoon ScriptHub v3 | WindUI Edition
-- by KHAFIDZKTP | github.com/HaZcK/ScriptHub
-- PASTE THIS BELOW THE JUNKIE TEMPLATE "end"
-- ════════════════════════════════════════════════
...
local function SkyMoonInit()
    ...
    local function runLoading(onComplete)
    ...
    local function openScanOverlay()
        ...
        task.spawn(function()
                ...
                for pid, data in pairs(db) do
                    ...
                    if ok and ok2 and info and myInfo and info.Creator and myInfo.Creator then
                        if info.Creator.Id == myInfo.Creator.Id then
                            entry = data
                            ap("✓ Sub-place verified — same game", "ffaa00")
                            break
                        end
                    end
                end
            ...
            task.wait(0.3)
            if entry then
                ap("✓ Game supported! Running script...", "00ffaa")
                task.wait(0.5)
                scanSg:Destroy()
                runScript(entry.script)
            else
                ap("✗ Game not supported.", "ff4444")
                task.wait(0.3)
                tp("Loading Universal Scripts...", "ffaa00", 0.05)
                task.wait(0.3)
                local udb = fetchUniversal()
                if udb then
                    output.Text = ""
                    ap("=== Universal Script List ===", "aaaaff")
    ...
    local function openMiniCmd()
        ...
        inputBox.Text = ""
        inputBox.PlaceholderText = "Check In [Service, Folder, Name]  |  RunConsole"
    ...
    local function startWindUI()
        ...
        if not ok then warn("[SkyMoon] WindUI failed: " .. tostring(lib)) return end
        WindUI = lib
        ...
        HT:Button({ Title="📋 Universal Scripts", Callback=function()
            local udb = fetchUniversal()
            if not udb then notify("❌ Error","Failed!") return end
            local names = {}
            for k, v in pairs(udb) do table.insert(names, v.name) end
            notify("Universal", table.concat(names, ", "):sub(1,100), 5)
        end})
        HT:Button({ Title="🔥 Real Builder", Callback=function()
            notify("⏳","Loading Real Builder...")
        ...
        MT:Toggle({ Title="Fly", Value=false, Callback=function(s)
            if s then
                ...
                local hrp=LP.Character.HumanoidRootPart bp.Position=hrp.Position bg.CFrame=hrp.CFrame
                bp.Parent=hrp bg.Parent=hrp _G.SkyFlyBP=bp _G.SkyFlyBG=bg
        ...
        BT:Button({ Title="⟳ Spawn Part", Callback=function() pcall(function() local p=Instance.new("Part") p.CFrame=workspace.CurrentCamera.CFrame*CFrame.new(0,0,-8) p.Size=Vector3.new(4,4,4) p.Anchored=true p.BrickColor=BrickColor.new("Bright blue") p.Parent=workspace end) notify("📦","Part spawned!") end})
